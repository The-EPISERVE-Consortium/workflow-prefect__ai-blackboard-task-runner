...
function cacheHitTitle(u) {
  ...
  return "hit rate = cacheRead / (input + cacheRead + cacheWrite)\n= " +
    fmtInt(cr) + " / (" + fmtInt(i) + " + " + fmtInt(cr) + " + " + fmtInt(cw) + ") = " + fmtInt(i + cr + cw) +
    "\ncacheWrite is newly-written cache content this step (counts as a miss), which lowers the hit rate";
}
...
function renderDetail(node) {
  ...
  pane.innerHTML = "";
  if (!node) {
    pane.appendChild(el("div", "detail-empty", "Select a node on the left to see details"));
    return;
  }
  const t = node.type;
  ...
  if (t === "session") {
    ...
    if (node.data.skillsLoaded && node.data.skillsLoaded.length) {
      ...
      if (node.data.skillsLoaded.length > 50) {
        const m = el("div", "no-data", "... " + (node.data.skillsLoaded.length - 50) + " more");
        wrap.appendChild(m);
      }
    ...
    if (d.thinking) {
      ...
      section("thinking", "Thinking", textBlock("(encrypted — model reasoned but content isn't readable)"));
    }
    ...
    if (d.toolsUsed && d.toolsUsed.length) {
      ...
      Object.entries(counts).sort((a,b)=>b[1]-a[1]).forEach(([n, c]) => {
        ...
        if (errs[n]) {
          ...
          setText(e, " (" + errs[n] + " err)"); row.appendChild(e);
        }
        wrap.appendChild(row);
      });
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (!hasAnyErrors(TRACE_DATA)) {
    ...
    onlyErrBtn.title = "No error nodes in this trace";
  } else {
